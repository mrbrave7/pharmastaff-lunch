...
async def init() -> None:
    ...
    # Migrate older databases: add payment_method to orders if it's missing.
    cur = await _conn.execute("PRAGMA table_info(orders)")
...
async def _build_lines(requested: list[tuple[int, int]]):
    """Snapshot name/price for each requested (item_id, qty>0). Returns (lines, total)."""
    lines = []
    total = 0
...
async def create_order(
    user_id: int, requested: list[tuple[int, int]], method: str = "card"
) -> dict:
    """Create an order. method 'cash' is auto-confirmed (no screenshot/admin review);
    'card' starts as pending_payment. Raises ValueError on active_order_exists /
    empty_cart / unknown_item.
    """
    status = "confirmed" if method == "cash" else "pending_payment"
    async with _write_lock:
        placeholders = ",".join("?" * len(ACTIVE_STATUSES))
        cur = await _conn.execute(
        ...
        lines, total = await _build_lines(requested)
        now = _now()
        cur = await _conn.execute(
            "INSERT INTO orders(user_id, status, payment_method, total, created_at, updated_at) "
            "VALUES (?, ?, ?, ?, ?, ?)",
            (user_id, status, method, total, now, now),
        )
...
async def update_order_items(
    user_id: int, order_id: int, requested: list[tuple[int, int]]
) -> dict:
    """Replace the items of a still-editable order (card pending, or cash confirmed)
    and recompute the total. Status/method are kept. Raises ValueError otherwise.
    """
    async with _write_lock:
        cur = await _conn.execute(
            "SELECT status, payment_method FROM orders WHERE id=? AND user_id=?",
            (order_id, user_id),
        )
        row = await cur.fetchone()
        if not row:
            raise ValueError("order_not_found")
        editable = row["status"] == "pending_payment" or (
            row["status"] == "confirmed" and row["payment_method"] == "cash"
        )
        if not editable:
            raise ValueError("locked")

        lines, total = await _build_lines(requested)
        await _conn.execute("DELETE FROM order_items WHERE order_id=?", (order_id,))
        await _conn.executemany(
            "INSERT INTO order_items(order_id, menu_item_id, name, unit_price, qty) "
            "VALUES (?, ?, ?, ?, ?)",
            [(order_id, iid, name, price, qty) for iid, name, price, qty in lines],
        ...
        await _conn.execute(
            "UPDATE orders SET total=?, updated_at=? WHERE id=?", (total, _now(), order_id)
        )
        await _conn.commit()
    return await get_order(order_id)
...
async def cancel_order(order_id: int, user_id: int) -> bool:
    """Cancel own order if it isn't a finalized card payment: allowed for card
    'pending_payment' and for cash 'confirmed' (cash is paid at pickup). A card order
    whose screenshot was sent (awaiting_review) or confirmed stays locked. Caller
    enforces the time cutoff.
    """
    async with _write_lock:
        cur = await _conn.execute(
            "UPDATE orders SET status='cancelled', updated_at=? "
            "WHERE id=? AND user_id=? AND "
            "(status='pending_payment' OR (status='confirmed' AND payment_method='cash'))",
            (_now(), order_id, user_id),
        )
        await _conn.commit()
        return cur.rowcount > 0
...
async def report_summary() -> dict:
    ...
    # Split confirmed money by payment method (card already paid vs cash to collect).
    cur = await _conn.execute(
        "SELECT payment_method AS m, COUNT(*) AS n, COALESCE(SUM(total), 0) AS money "
        "FROM orders WHERE status='confirmed' GROUP BY payment_method"
    )
    by = {r["m"]: {"n": r["n"], "money": r["money"]} for r in await cur.fetchall()}
    return {
        "dishes": dishes,
        "order_count": totals["n"],
        "money": totals["money"],
        "unpaid": unpaid,
        "card": by.get("card", {"n": 0, "money": 0}),
        "cash": by.get("cash", {"n": 0, "money": 0}),
    }
